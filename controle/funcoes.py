...
def is_traje_disponivel(traje, MostraAlocados):
    ...
    statusLocacao = []
    if not itens_usados and MostraAlocados:
    ...
    for item_usado in itens_usados:
        ...
        for location in locados:
            statusLocacao.append(location.status)

    if MostraAlocados:
        if 'Alocado' in statusLocacao or 'Atraso' in statusLocacao:
            return traje
    else:
        if ('Alocado' in statusLocacao or 'Atraso' in statusLocacao) and traje.ativo == 'sim':
            return None
        elif traje.ativo == 'nao':
            return None
        else:
            return traje
...
def busca_locacao_por_cliente(cliente):
    ...
    for locacoes in cliente.locacao_set.select_related().values():
        ...
        locacaoRelacionados = Locacao.objects.filter(
            id=locacoes['id']).select_related()
...
def criar_locacao(cliente, dataPrevisao, listaTrajes, valorTotal):
    try:
        lanc = Lancamento(data=date.today(), hora=str(
            datetime.now().hour)+':'+str(datetime.now().minute), valor=valorTotal)
        lanc.save()
        ...
        for traje in listaTrajes:
            ...
            item = Item(valor=traje['valor'],
                        ...
                        data_entrega=datetime.now().strftime(
                            '%Y-%m-%d %H:%M:%S') if traje['precisaAjuste'] == False else None,
                        medidas=None if 'id_medida' not in traje else Ficha.objects.get(
                            id=traje['id_medida']),
                        traje=traje_retorno)
            item.save()
...
def criar_cliente(info):
    if info['isEstrangeiro'] == True:
        cliente = Cliente(nome=info["Nome"].strip(), email=info["Email"], endereco=info["Endereco"],
                          telefone=info["Telefone"], documento_externo=info["DocumentoExterno"])
    else:
        cliente = Cliente(nome=info["Nome"].strip(), email=info["Email"],
                          endereco=info["Endereco"], telefone=info["Telefone"], rg=info["RG"], cpf=info["CPF"])
    cliente.save()
