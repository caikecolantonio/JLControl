...
def avisa_trajes(locacao, cliente, lista):
    try:
        ...
        for item in lista:
            message = message + f"{item.traje.nome} de modelo {item.traje.modelo} com corte {item.traje.corte} ficou pronto em {item.data_entrega.astimezone(timezone(timedelta(hours=-3)))}.\n"
